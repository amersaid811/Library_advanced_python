...
######################################FRAMES######################################################
        ...
        #heading
        self.top_image= PhotoImage(file='icons/editbook.png')
        top_image_lbl=Label(self.topframe,image=self.top_image,bg='white')
        top_image_lbl.place(x=120,y=20)
        heading=Label(self.topframe, text ='   Edit Book   ',font='arial 22 bold',fg='#003f8a',bg='white')
...
######################################LABELS######################################################
        ...
        self.ent_shelf_location.place(x=300, y=285)
        #button
        button=Button(self.bottomFrame,text='Edit Book',command=self.editBook)
    ...
    def editBook(self):
        ...
        old_quantity = result[6]  # Assuming Quantity is at index 6 in the result tuple
        new_quantity = int(book['Quantity'])

        query = "UPDATE lib.books SET Title = %s, Author = %s, ISBN = %s, Genre = %s, ShelfLocation = %s, Quantity = %s WHERE BookID = %s"
        data = (book['Title'], book['Author'], book['ISBN'], book['Genre'], book['ShelfLocation'], new_quantity, bookid)

        try:
            cursor.execute(query, data)
            
            # Update status if quantity changed from 0 to non-zero or vice versa
            if  new_quantity > 0:
                status_query = "UPDATE lib.books SET Status = 'Available' WHERE BookID = %s"
                cursor.execute(status_query, (bookid,))
            else:
                status_query = "UPDATE lib.books SET Status = 'Sold Out' WHERE BookID = %s"
                cursor.execute(status_query, (bookid,))
